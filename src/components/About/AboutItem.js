...
const AboutItem = ({ image, icon, title, description }) => {
  return (
    <div
      ...
      className="bg-white transform transition duration-700 hover:scale-105 p-6 rounded-2xl hover:shadow-xl"
    >
      {/* image  */}
      <div className="overflow-hidden rounded-2xl flex flex-grow">
        <img
          className=" bg-contain bg-center transform transition duration-700 hover:scale-125"
          src={image}
          alt={title}
        />
      </div>
